...
let () =
  ...
  Mesh.LaTeX.save mesh "/tmp/testmesh.tex";
  ...
  let in_black l = List.map (fun l -> (l, "black")) l in
  Mesh.LaTeX.level_curves ~boundary:(fun _ -> Some "red")
    mesh z (in_black [-0.5; -0.2; 0.; 0.1; 0.5; 0.8; 0.95]) "/tmp/levels.tex"



(* Local Variables: *)
(* compile-command: "make -k test.exe" *)
(* End: *)
